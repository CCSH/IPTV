...
def updateChannelUrlsM3U(channels, template_channels, name):
    written_urls = set()
    ...
    with open(name + ".m3u", "w", encoding="utf-8") as f_m3u:
        f_m3u.write(f"""#EXTM3U x-tvg-url={",".join(f'"{epg_url}"' for epg_url in config.epg_urls)}\n""")

        with open(name + ".txt", "w", encoding="utf-8") as f_txt:
            for group in config.announcements:
                    ...
                    for channel_name in channel_list:
                        if channel_name in channels[category]:
                            ...
                            for index, url in enumerate(filtered_urls, start=1):
                                ...
                                f_m3u.write(f"#EXTINF:-1 tvg-id=\"{index}\" tvg-name=\"{channel_name}\" tvg-logo=\"https://epg.112114.free.hr/logo/{channel_name}.png\" group-title=\"{category}\",{channel_name}\n")
                                f_m3u.write(new_url + "\n")
